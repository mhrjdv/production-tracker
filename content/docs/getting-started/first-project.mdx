...
This guide walks you through creating your first Laserman project, uploading a screenplay, and reviewing the AI-parsed scene structure. By the end, you'll have a fully structured project ready for shot planning and asset capture.

## Understanding Projects

In Laserman, a **Project** is the top-level container for your film or series. Each project contains:
...
A single Laserman instance can manage multiple projects simultaneously, making it suitable for production studios working on several films or episodes at once.

## Step 1: Log In to Laserman
...
- **Password**: Set via NextAuth provider

> **Note:** NextAuth v5 supports multiple authentication providers. The default setup uses email/password, but you can configure OAuth providers like Google or GitHub in `src/app/api/auth/[...nextauth]/route.ts`.
...
```
Example: "Laserman V2: The Awakening"
```
...
Laserman accepts screenplays in the following formats:
...
Laserman uses OpenRouter (configured in `.env`) to:
...
Laserman uses a hierarchical shot naming convention:
...
3. **Capture Versions**: Use the Chrome extension to save generated assets to Laserman
...
You've successfully created your first Laserman project and uploaded a structured screenplay. Your scenes and shots are now ready for asset generation and capture.

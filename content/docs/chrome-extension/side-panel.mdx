...
The Laserman extension uses Chrome's Side Panel API to provide a persistent workspace alongside AI platform pages. Unlike traditional popups that close when you click away, the side panel stays open, enabling continuous capture workflow.
...
- **Context Selectors** - Choose project and scene
...
- Retrying failed syncs (up to 5 retries per item)
...
- **App URL** - Your Laserman instance URL
- **API Token** - Authentication token (starts with `lmt_`)
...
3. Select project and scene (if not already set)
...
| Disconnected | Red | Cannot reach Laserman API or invalid token |

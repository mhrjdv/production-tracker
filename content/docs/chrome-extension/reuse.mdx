...
> Drafts persist across browser sessions but are stored locally (not synced to Laserman).
...
> History mode pulls data from the Laserman API in real-time when the scene context changes.
...
- Laserman stores full prompts (no truncation)

...
function prismaStore(): ShotStore {
    return {
        ...
        async create(data) {
            return prisma.shot.create({ data: data as Prisma.ShotUncheckedCreateInput });
        },
        async update(id, data) {
            return prisma.shot.update({ where: { id }, data: data as Prisma.ShotUncheckedUpdateInput });
        },
        async delete(id) {
...
export async function createShotAction(sceneId: string, input: CreateShotInput) {
...
export async function reorderShotsAction(sceneId: string, orderedIds: string[]) {
    const session = await auth();
    if (!session?.user?.id) throw new Error("Unauthorized");

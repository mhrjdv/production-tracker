...
import {
  findPromptPackagesBySceneId,
  findSceneAssetsBySceneId,
...
async function SceneDetailContent({
  ...
  const [
    ...
    prisma.character.findMany({
      where: { projectId },
      orderBy: { name: "asc" },
      select: { id: true, name: true, role: true, portraitUrl: true },
    }),
    prisma.sceneCharacter.findMany({
      where: { sceneId: scene.id },
      select: { characterId: true },
    }),
    prisma.shotCharacter.findMany({
      where: { shot: { sceneId: scene.id } },
      select: { shotId: true, characterId: true },
    }),
  ]);
  ...
  return (
    <SceneDetailClient
      ...
      sceneCharacterIds={sceneCharacters.map((sc) => sc.characterId)}
      shotCharactersMap={shotCharacters.reduce<Record<string, string[]>>(
        (acc, sc) => {
          const list = acc[sc.shotId] ?? [];
          return { ...acc, [sc.shotId]: [...list, sc.characterId] };
        },
        {},
      )}
      prev={prev}

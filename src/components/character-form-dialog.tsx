...
export function CharacterFormDialog({
  ...
  const [designPhilosophy, setDesignPhilosophy] = useState(
    character?.designPhilosophy ?? "",
  );
  const [visualCues, setVisualCues] = useState<string[]>(
    character?.visualCues ?? [],
  );
  const [bodyLanguage, setBodyLanguage] = useState<string[]>(
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ...
              : "Add a new character to this project."}
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {visualCues.map((cue, i) => (
                  <Badge key={i} variant="secondary" className="gap-1 pr-1">
                    ...
                    <button
                      onClick={() => removeTag(visualCues, setVisualCues, i)}
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {bodyLanguage.map((lang, i) => (
                  <Badge key={i} variant="outline" className="gap-1 pr-1">
                    ...
                    <button
                      onClick={() =>
                        removeTag(bodyLanguage, setBodyLanguage, i)
                      }
                      className="ml-0.5 hover:text-destructive"
                    >
                      <X className="h-3 w-3" />

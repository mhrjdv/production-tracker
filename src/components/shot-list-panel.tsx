...
import { updateShotCharacters } from "@/lib/actions/shot-character-actions";
import {
  Plus,
  Pencil,
  Trash2,
...
function ShotForm({
  ...
  const set =
    (field: keyof Omit<ShotFormValues, "characterIds">) =>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {projectCharacters.map((char) => {
                const isSelected = values.characterIds.includes(char.id);
                return (
                  <button
                    key={char.id}
                    type="button"
...
export function ShotListPanel({
  ...
  const onCreate = (values: ShotFormValues) => {
    startTransition(async () => {
      try {
        setFormError(null);
        const shot = await createShotAction(sceneId, {
        ...
        if (values.characterIds.length > 0) {
          await updateShotCharacters(shot.id, values.characterIds);
        }
        setIsAdding(false);
      } catch (error) {
        setFormError(
  ...
  const onUpdate = (shotId: string, values: ShotFormValues) => {
    startTransition(async () => {
      try {
        ...
        await updateShotCharacters(shotId, values.characterIds);

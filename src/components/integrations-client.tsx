...
export function IntegrationsClient({
  ...
  const confirmRevoke = () => {
    ...
    startTransition(async () => {
      await revokeExtensionApiToken(revokeTokenId);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">
            Chrome Extension Access Token
          </CardTitle>
          <CardDescription>
            Create a personal token once, paste it in the extension popup, and
            sync scene versions from any AI platform tab.
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Last synced project/scene/platform and model defaults saved on your
            profile.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 text-sm md:grid-cols-2">
            <div className="rounded-lg border p-3">
              ...
              <p className="font-mono text-xs">
                {extensionPreferences.lastProjectId || "—"}
              </p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground">Scene ID</p>
              <p className="font-mono text-xs">
                {extensionPreferences.lastSceneId || "—"}
              </p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground">Platform</p>
              <p className="font-mono text-xs">
                {extensionPreferences.lastPlatform || "—"}
              </p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground">
                Preferred Type / Status
              </p>
              <p className="font-mono text-xs">
                {extensionPreferences.preferredAssetType || "—"} /{" "}
                {extensionPreferences.preferredStatus || "—"}
              </p>
            </div>
            <div className="rounded-lg border p-3 md:col-span-2">
              ...
              <p className="font-mono text-xs break-all">
                {extensionPreferences.openAiBaseUrl || "—"}{" "}
                {extensionPreferences.openAiModel
                  ? `· ${extensionPreferences.openAiModel}`
                  : ""}
              </p>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Revoke old devices and rotate credentials regularly.
          </CardDescription>
        </CardHeader>
      ...
      <ConfirmDialog
        ...
        title="Revoke token?"
        description="The extension will stop syncing until you update it with a new token."
        confirmLabel="Revoke"
        variant="destructive"
